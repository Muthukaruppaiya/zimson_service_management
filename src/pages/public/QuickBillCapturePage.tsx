...
export function QuickBillCapturePage() {
  ...
  function validateCaptureFile(file: File, kind: string): string | null {
    if (file.size > WATCH_ATTACHMENT_MAX_BYTES) {
      return `File is too large (${(file.size / (1024 * 1024)).toFixed(1)} MB). Max ${watchAttachmentMaxSizeLabel()}.`;
    }
    if (kind === SRF_DOCUMENT_PHOTO_KIND) {
      const mime = (file.type || "").toLowerCase();
      if (mime.startsWith("image/")) return validateQuickBillImageFile(file);
      return validateQuickBillDocumentFile(file);
    }
    return validateQuickBillImageFile(file);
  }

  async function uploadFile(file: File, kind: string) {
    if (!token) return;
    setUploadError(null);
    const formatErr = validateCaptureFile(file, kind);
    if (formatErr) {
  ...
  async function removePhoto(photoId: string) {
    ...
    try {
      const data = await apiJson<CaptureSession>(
        `/api/public/quick-bill-capture/photo/${encodeURIComponent(photoId)}?token=${encodeURIComponent(token)}`,
        { method: "DELETE" },
      );
      setSession(data);
    ...
    <div className="min-h-screen bg-stone-100 px-4 py-6 text-stone-900">
      <div className="mx-auto max-w-md">
        ...
        <input
          ...
          type="file"
          accept={`${WATCH_DOCUMENT_ACCEPT},${WATCH_IMAGE_ACCEPT}`}
          className="sr-only"
          tabIndex={-1}
          disabled={!canUpload || busy}
