...
} from "../../../lib/customerBulkImportColumns";
...
function ColumnTable({
  ...
  badge: string;
  desc: string;
  columns: typeof CUSTOMER_BULK_IDENTITY_COLUMNS;
}) {
  return (
    <div className="border border-rlx-rule bg-white overflow-hidden">
      <div className="flex items-center justify-between bg-rlx-green px-4 py-3 text-white">
        ...
        <span className="text-[10px] font-semibold opacity-70">{badge}</span>
      </div>
      <p className="border-b border-rlx-rule px-4 py-2 text-[11px] text-stone-500">{desc}</p>
...
type PreviewRow = { phone: string; name: string; kind: string; action: "create" | "update" };

export function CustomerBulkImportPage() {
  ...
  const downloadTemplate = useCallback(async () => {
    ...
    setMsg(null);
    try {
      const res = await fetch("/api/customers/bulk-import/template", { credentials: "include" });
      if (!res.ok) {
      ...
      a.href = url;
      a.download = "zimson_customer_bulk_import.xlsx";
      a.click();
      URL.revokeObjectURL(url);
      setMsg({
        type: "ok",
        text: "Template downloaded. Sample rows are examples — delete them before a live import. SMS and email OTP are not sent on bulk import. Email is optional.",
      });
    } catch (e) {
  ...
  const validate = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/customers/bulk-import/validate", {
        method: "POST",
        body: fd,
        credentials: "include",
      });
      ...
      setMsg({ type: "ok", text: "File looks good. Review the preview below, then import. No SMS or email OTP will be sent." });
    } catch {
  ...
  const commit = useCallback(async () => {
    ...
    try {
      const fd = new FormData();
      fd.append("file", fileRef.current);
      const res = await fetch("/api/customers/bulk-import/commit", {
        method: "POST",
        body: fd,
        credentials: "include",
      });
  ...
  if (!canImport) {
    return (
      <div>
        ...
        <PageHeader title="Customer Bulk Import" description="" />
        <div className="border border-rlx-rule bg-white px-6 py-10 text-center text-sm text-stone-400">
          You do not have permission to run customer bulk import.
          <div className="mt-4">
            <Link to="/service/customers/master" className="font-semibold text-rlx-green hover:underline">
  ...
  return (
    <div>
      ...
      <BulkImportSuccessModal
        ...
        description="Customers have been saved. No SMS or email OTP was sent."
        stats={
          importResult
            ? [
        ...
        masterHref="/service/customers/master"
        ...
        <div className="mb-6 border border-blue-200 bg-blue-50 px-5 py-5">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 text-xs text-blue-700">No SMS or email verification was sent. Email remains optional.</p>
          ...
          <div className="border border-rlx-rule bg-white">
            ...
            <div className="p-5">
              <p className="text-sm text-stone-600">
                Excel with a <strong>README</strong> and a <strong>Customers</strong> sheet covering identity, tax, billing,
                shipping, and two extra addresses — the same fields as manual registration.
              </p>
              <ul className="mt-3 space-y-1 text-xs text-stone-500">
                ...
                <li className="flex items-center gap-2">
                  <span className="font-bold text-rlx-green">✓</span> No SMS or email OTP on bulk import
                </li>
                <li className="flex items-center gap-2">
                  <span className="font-bold text-rlx-green">✓</span> Matching primary mobile updates; new mobiles create
                </li>
                <li className="flex items-center gap-2">
      ...
      {validated && preview.length > 0 && !importResult && (
        <div className="mt-5 border border-rlx-rule bg-white">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {preview.map((row) => (
                <tr key={row.phone} className="border-t border-rlx-rule">
                  <td className="px-5 py-2 font-mono text-xs text-stone-700">{row.phone}</td>
                  <td className="px-5 py-2 text-stone-800">{row.name}</td>
                  <td className="px-5 py-2 text-stone-600">{row.kind}</td>
      ...
      <div className="mt-8">
        ...
        <div className="grid gap-4 lg:grid-cols-1">
          ...
          <ColumnTable
            ...
            badge="Same sheet"
            desc="Billing address is required. Set Same Shipping As Billing to Y to copy billing, or fill shipping columns."
            columns={CUSTOMER_BULK_BILLING_COLUMNS}
          />
          <ColumnTable
            title="Additional addresses"
            badge="Optional"
            desc="Up to two extra addresses. Leave blank to skip. If any field is filled, the extra address must be complete."
            columns={CUSTOMER_BULK_EXTRA_COLUMNS}
          />
        </div>
